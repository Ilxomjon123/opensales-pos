...
async function run() {
  ...
  catch (e: any) { notify('Yangilashda xato: ' + (e?.message ?? e), 'error') }
}
</script>
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-center justify-between">
          <div class="text-sm font-semibold">Yangi versiya {{ updateInfo.version }}</div>
        ...
        <div v-if="updating" class="mt-3">
          ...
          <div class="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground"><RefreshCw class="h-3 w-3 animate-spin" /> Yuklanmoqda… {{ updateProgress }}%</div>
        </div>

        <div v-else class="mt-3 flex gap-2">
          <button @click="run" class="flex h-8 flex-1 items-center justify-center gap-1.5 rounded-lg bg-primary text-xs font-medium text-primary-foreground hover:bg-primary/90"><Download class="h-3.5 w-3.5" /> Yangilash</button>
          <button @click="dismissUpdate" class="h-8 rounded-lg border px-3 text-xs hover:bg-muted">Keyinroq</button>
        </div>
      </div>
    </div>
  </div>
